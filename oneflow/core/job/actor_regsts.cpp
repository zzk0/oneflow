...
namespace oneflow {
void Map2Csv(const std::string& filepath,
             const HashMap<int64_t, std::vector<int64_t>>& actor_id2regsts) {
  std::ofstream out_stream(filepath);
  out_stream << "actor,regsts.../"
             << "\n";
  for (auto pair : actor_id2regsts) {
...
void ActorRegsts(const std::string& plan_filepath,
                 const std::string& report_folder) {
  Global<JobDesc>::New();
  ...
  Map2Csv(JoinPath(report_folder, "produced_regsts.csv"),
          actor_id2produced_regsts);
  Map2Csv(JoinPath(report_folder, "consumed_regsts.csv"),
          actor_id2consumed_regsts);
