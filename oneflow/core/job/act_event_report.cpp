...
void ActEventReport(const std::string& plan_filepath,
  ...
  for (auto event : *act_events) {
    double t = GetDiffTime(event.actor_id(), machine_time_diffs);
    ...
    out_stream << "'" << Time2String(event.push_time() - t) + ",";
    out_stream << "'" << Time2String(event.start_time() - t) + ",";
    out_stream << "'" << Time2String(event.stop_time() - t) + ",";
