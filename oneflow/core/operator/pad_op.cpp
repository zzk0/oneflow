...
class PadOp final : public Operator {
 public:
  ...
  Maybe<void> InferBlobDescs(std::function<BlobDesc*(const std::string&)> GetBlobDesc4BnInOp,
    ...
    const int32_t padding_w = op_conf().pad_conf().pad_left() + op_conf().pad_conf().pad_right();
    const int32_t padding_h = op_conf().pad_conf().pad_top() + op_conf().pad_conf().pad_bottom();
    out_blob->mut_shape() =
