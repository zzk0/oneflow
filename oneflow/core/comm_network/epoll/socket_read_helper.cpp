...
void SocketReadHelper::SetStatusWhenMsgBodyDone() {
  if (cur_msg_.msg_type == SocketMsgType::kPleaseRead) {
    Global<EpollCommNet>::Get()->ReadDone(cur_msg_.please_read_msg.read_id);
  }
  SwitchToMsgHeadReadHandle();
}

void SocketReadHelper::SetStatusWhenPleaseWriteMsgHeadDone() {
  SocketMsg msg_to_send;
  msg_to_send.msg_type = SocketMsgType::kPleaseRead;
  msg_to_send.please_read_msg.src_token = cur_msg_.please_write_msg.src_token;
  msg_to_send.please_read_msg.dst_token = cur_msg_.please_write_msg.dst_token;
  msg_to_send.please_read_msg.read_id = cur_msg_.please_write_msg.read_id;
  Global<EpollCommNet>::Get()->SendSocketMsg(cur_msg_.please_write_msg.dst_machine_id,
                                             msg_to_send);
  SwitchToMsgHeadReadHandle();
}

void SocketReadHelper::SetStatusWhenPleaseReadMsgHeadDone() {
  auto mem_desc = static_cast<const SocketMemDesc*>(cur_msg_.please_read_msg.dst_token);
