...
__global__ void CalIoUMatrixKernel(const T* boxes1_ptr, const int32_t num_boxes1,
  ...
  for (size_t i = blockIdx.x; i < num_boxes1; i += gridDim.x) {
    for (size_t j = threadIdx.x; j < num_boxes2; j += blockDim.x) {
      ...
      const T box1_y1 = boxes1_ptr[i * 4];
      const T box1_x1 = boxes1_ptr[i * 4 + 1];
      const T box1_y2 = boxes1_ptr[i * 4 + 2];
      const T box1_x2 = boxes1_ptr[i * 4 + 3];
      const T box2_y1 = boxes2_ptr[j * 4];
      const T box2_x1 = boxes2_ptr[j * 4 + 1];
      const T box2_y2 = boxes2_ptr[j * 4 + 2];
      const T box2_x2 = boxes2_ptr[j * 4 + 3];
