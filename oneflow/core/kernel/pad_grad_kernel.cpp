...
class PadGradKernel final : public KernelIf<device_type> {
 ...
 private:
  void ForwardDataContent(const KernelCtx& ctx,
    ...
    const int64_t pad_top = this->op_conf().pad_grad_conf().pad_top();
    const int64_t pad_left = this->op_conf().pad_grad_conf().pad_left();
    std::vector<int64_t> src_pos_vec = {0, pad_top, pad_left * size_of_data_type};
    std::vector<int64_t> dst_pos_vec = {0, 0, 0};
