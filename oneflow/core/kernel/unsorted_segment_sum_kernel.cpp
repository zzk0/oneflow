...
void UnsortedSegmentSumKernel<device_type, T, K>::ForwardDataContent(
  ...
  const UnsortedSegmentSumOpConf& conf = this->op_conf().unsorted_segment_sum_conf();
  const int64_t axis = conf.axis();
  const int64_t outer_dim_size = out_shape.Count(0, axis);
  const int64_t num_segments = conf.num_segments();
  ...
  if (conf.no_duplicates_in_segment_ids()) {
    UnsortedSegmentSumKernelUtil<device_type, T, K>::UnsortedSegmentSumNoDuplicates(
