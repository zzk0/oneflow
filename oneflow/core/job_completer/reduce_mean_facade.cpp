...
void GenerateFacadeImplOpConf(const OpNode& op_node, JobBuilder* job_builder) {
  ...
  *reduce_sum_conf->mutable_axis() = reduce_mean_conf.axis();
  ...
  //    } else {
  //      *partial_elem_cnt_conf->mutable_include_axis_conf()->mutable_axis() =
  //      reduce_mean_conf.axis();
  //    }
  ...
  //    boradcast_div_op_conf.set_name("System-Facade-" + op_node.op().op_name() +
  //    "_broadcast_div"); auto* boradcast_div_conf =
  //    boradcast_div_op_conf.mutable_broadcast_div_conf();
