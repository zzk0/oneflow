...
class MatmulBiasaddFloatingKernel final : public user_op::OpKernel {
 public:
  MatmulBiasaddFloatingKernel() = default;
  ~MatmulBiasaddFloatingKernel() = default;
 ...
 private:
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    NewKernelUtil<device_type>::OFMatmulBiasadd(ctx->device_ctx(), trans_a, trans_b, m, n,
                                                k, GetOneVal<T>(), a->dptr<T>(), b->dptr<T>(),
                                                beta, outer_size, bias_size, inner_size,
                                                bias->dptr<T>(), out->mut_dptr<T>());
  }
};

#define REGISTER_MATMUL_BIASADD_KERNEL(device, dtype)                                           \
  REGISTER_USER_KERNEL("matmul_biasadd")                                                        \
      .SetCreateFn<MatmulBiasaddFloatingKernel<device, dtype>>()                                \
...
//  public:
//   MatmulBiasaddGpuHalfKernel() = default;
//   ~MatmulBiasaddGpuHalfKernel() = default;
