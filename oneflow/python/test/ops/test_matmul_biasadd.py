...
def compare_with_tensorflow(
    ...
    def MatmulBiasaddJob():
        with flow.scope.placement(device_type, "0:0"):
            ...
            if data_type == "float16":
                ...
                out = flow.matmul_biasadd(a, b, bias, transpose_a, transpose_b, data_format)
                c = flow.get_variable(
                    "c",
                    shape=out.shape,
                    dtype=dtype,
            ...
            flow.watch(b, test_global_storage.Setter("bias"))
            flow.watch_diff(b, test_global_storage.Setter("bias_diff"))
    ...
    #     tf_out = tf.nn.bias_add(tf_out, bias, data_format)
    #     tf_out = tf_out + c
    #     if data_type == "float16":
...
class TestMatmul(flow.unittest.TestCase):
    def test_matmul(test_case):
        for arg in gen_arg_list():
            if arg[0] == "cpu" and (arg[7] == "float16" or arg[9] == True):
                continue
            if arg[7] != "float32" and arg[9] == True:
                continue
